...
#define   Motor1L_on          output_bit(PIN_C4,1)
#define   Motor1L_off         output_bit(PIN_C4,0)
#define   Motor1H_on          output_bit(PIN_C5,1)
#define   Motor1H_off         output_bit(PIN_C5,0)

#define   Motor2L_on          output_bit(PIN_C6,1)
#define   Motor2L_off         output_bit(PIN_C6,0)
#define   Motor2H_on          output_bit(PIN_C7,1)
#define   Motor2H_off         output_bit(PIN_C7,0)
...
char  Baje_Vidrio[] = "!!Ubique Vidrio!!",Inflowt[] = "Exflow(m/s):",Downflowt[] = "Dnflow(m/s):", ZF[] = "Fabrica:207.0", ZA[] = "Actual:",AJ[]="Ajuste:";
char  Vida_UV[] = "UVLife:",Vida_Filtro[] = "Filter:",ZFP[] = "Fabrica:235.0", Perdida[]="!Flujo Bajo!";
char Ahorro[] = "Modo ECO";
int8 Menu=200, unidad=1, Opcion=1,inicie=0,negativo=10,G_l=0,G_h=0,tiemporeset=0,minutos_uv=0,Pinicial=0,Pfinal=0;
int8 q=0,r=0,l=0,h=0,Entero=0,Decimal1=0,LuzUV=10,Motor=10,Alarma=10,Alarma2=10,UV2=0,e=0,m=0,VelMotor=0;
...
float ADC=0,Temporal=0.0,Temporal2=0.0,Pini=0.0,Pfin=0.0;
float a=0.0000001418633343546420,b=-0.00009476749001431169,c= 0.040182822903506,d= -5.354852229527197;//Sensor 2 Posible bueno
//float a=-0.000021424,b=0.013616,c=-2.8520,d=196.96;//Ecuacion con Hilo Caliente Patron
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      { 
         ...
         if(VIDRIOUP){
            VelocidadMotor(VelMotor);tiempo_purga=0;glcd_text57(10,55, Baje_Vidrio, 1, ON);
         }
...
// Dibujo de barra de estado de filtro                  
         //barra=100.0*(Filtro_Downflow/0.75);
         barra=100.0*((Filtro_Downflow-Pini)/Pfin);
         if(barra>100.0)
            barra=100.0;
         if(barra<0.0)
            barra=0.0;
...
// Fin de Dibujo de barra de estado de Luz UV
         ...
         if(RIGHT){//Si oprime boton de Blower.
            if(Flanco3 == 0){
               Flanco3 = 1;delay_ms(1);flag_blower=!flag_blower;
            }
         }else{
            Flanco3 = 0;
         }
         ...
         if(LEFT){//Si oprime boton de Luz UV.         
            if(Flanco4 == 0){
               Flanco4 = 1;delay_ms(1);flag_uv=!flag_uv;
               if(UV2==0)
                  UV2=10;
               else
                  UV2=0;
         ...
         if(DOWN){//Si oprime boton de Luz Blanca.
            if(Flanco5 == 0){
               Flanco5 = 1;delay_ms(1);flag_luz=!flag_luz;
            }
         }
         else{
            Flanco5 = 0;
         ...
         if((Tempouv[0]>=Tempouvp[0])&&(Tempouv[1]>=Tempouvp[1])&&(Tempouv[2]>=Tempouvp[2])&&(Tempouv[3]>=Tempouvp[3]))      {
            if((Tempouvp[0]==0)&&(Tempouvp[1]==0)&&(Tempouvp[2]==0)&&(Tempouvp[3]==0)){
               ...
               Tempouv[0]=0;Tempouv[1]=0;Tempouv[2]=0;Tempouv[3]=0;UV2=0;LuzUV_off;
               for(i=0;i<5;i++){
                  Alarma_on;delay_ms(200);
                  Alarma_off;delay_ms(200);
               }
            }
         }
      }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(Opcion<=4){
         ...
         if(Opcion==5)   
            {strcpy(Menus.Letra,"Presion Inicial ");
            displayMenu(Menus.Letra,0,18,1,1);}
         else
            {strcpy(Menus.Letra,"Presion Inicial ");
            displayMenu(Menus.Letra,0,18,0,1);} 
         ...
         if(Opcion==6)   
            {strcpy(Menus.Letra,"Presion Final    ");
            displayMenu(Menus.Letra,0,29,1,1);}
         else
            {strcpy(Menus.Letra,"Presion Final    ");
            displayMenu(Menus.Letra,0,29,0,1);}  
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==45){ //Menu de Presion Inicial
      
      if(Pinicial>100)Pinicial=20;
      if(Pinicial<20)Pinicial=100;
      
      strcpy(Menus.Letra,"P. Inicial");
      displayMenu(Menus.Letra,10,0,1,1);
      
      displayfloat(Pinicial,44,30,0,2);
      
      if(UP)//Si oprime hacia arriba
      {  
         delay_ms(100);Pinicial+=1;
      }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==46){ //Menu de Presion Final
      
      if(Pfinal>100)Pfinal=10;
      if(Pfinal<10)Pfinal=100;
      
      strcpy(Menus.Letra,"P. Final");
      displayMenu(Menus.Letra,10,0,1,1);
      
      displayfloat(Pfinal,44,30,0,2);
      
      if(UP)//Si oprime hacia arriba
      {  
         delay_ms(100);Pfinal+=1;
      }
